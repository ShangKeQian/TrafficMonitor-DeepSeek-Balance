...
double ExtractFloat(const std::string& json, const char* key)
{
...
FetchResult FetchBalance(const std::wstring& api_key, BalanceInfo& out)
{
    ...
    WinHttpHandle hSession{ WinHttpOpen(L"DeepSeekPlugin/1.0",
    ...
    WinHttpHandle hConnect{ WinHttpConnect(hSession, L"api.deepseek.com",
        INTERNET_DEFAULT_HTTPS_PORT, 0) };
    if (!hConnect)
        return FetchResult::NetworkError;

    WinHttpHandle hRequest{ WinHttpOpenRequest(hConnect, L"GET", L"/user/balance",
    ...
    if (body.find("\"is_available\"") != std::string::npos &&
        ...
        out.topped_up_balance = 0.0;
        out.granted_balance = 0.0;
    }
